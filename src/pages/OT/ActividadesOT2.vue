      ...
      <div class="gantt-container bg-white rounded-lg shadow-md p-2 md:p-4 w-full h-full">
        <div class="overflow-x-auto pb-4">
          <div class="inline-block min-w-full">
            <!-- Cabecera de semanas -->
            <div class="border-b border-gray-300 rounded-t-lg grid grid-cols-5">
              <div
                class="col-span-1 sticky left-0 bg-white z-20 border-r border-gray-300 flex items-center justify-between px-3">
                <div class="flex space-x-2">
                  <button @click="prevWeek"
                    class="px-2 py-1 bg-white text-gray-700 hover:bg-gray-50 transition-colors flex items-center justify-center border border-gray-200 rounded-l-md">
                    <span class="material-icons text-sm" title="Semana Anterior">arrow_back</span>
                  </button>
                  <button @click="goToCurrentWeek"
                    class="px-2 py-1 bg-white text-gray-700 hover:bg-gray-50 transition-colors flex items-center justify-center border border-gray-200">
                    <span class="material-icons text-sm text-green-600" title="Semana Actual">today</span>
                  </button>
                  <button @click="nextWeek"
                    class="px-2 py-1 bg-white text-gray-700 hover:bg-gray-50 transition-colors flex items-center justify-center border border-gray-200 rounded-r-md">
                    <span class="material-icons text-sm" title="Semana Siguiente">arrow_forward</span>
                  </button>
                  <button @click="refreshData"
                    class="ml-1 px-2 py-1 bg-white text-indigo-600 rounded-md hover:bg-indigo-50 transition-colors border border-indigo-200 flex items-center justify-center">
                  ...
                  <button @click="openNewTaskModal"
                    class="ml-3 px-2 py-1 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-md hover:from-blue-600 hover:to-blue-700 transition-all shadow-sm flex items-center justify-center group">
                    <span class="material-icons text-sm mr-1 group-hover:scale-110 transition-transform">add</span>
                  </button>
                </div>
              </div>
              <div class="col-span-4 grid grid-cols-4">
                <div v-for="i in weeksToShow" :key="i"
                  class="text-center py-1.5 bg-blue-100 font-semibold text-blue-900 border-l border-gray-50 flex flex-col justify-center">
                  <span class="text-sm">Semana {{ getWeekLabel(i - 1) }}</span>
              ...
              <div class="col-span-4 grid grid-cols-4">
                <div v-for="weekIndex in weeksToShow" :key="weekIndex" class="flex">
                  <div v-for="n in 7" :key="n" :style="{ width: dayWidth + 'px' }" :class="[
                    'text-center py-1 text-sm font-medium border-l border-gray-300',
                    isWeekend(getDayDate(weekIndex, n)) ? 'bg-gray-100 text-gray-500' : 'text-gray-800',
                    isToday(getDayDate(weekIndex, n)) ? 'bg-blue-50 font-bold' : ''
                  ]">
                    <div class="text-xs"
                      :class="isWeekend(getDayDate(weekIndex, n)) ? 'text-gray-400' : 'text-gray-500'">
                      {{ formatWeekday(getDayDate(weekIndex, n)) }}
                    </div>
                    <div class="relative">
                      <div class="text-xs" :class="isToday(getDayDate(weekIndex, n)) ? 'text-blue-600' : ''">
            ...
            <div class="relative overflow-x-auto">
              <!-- Línea roja del día actual (se muestra solo si diffDays >= 0 y menor que los días mostrados) -->
              <div v-if="currentDayLeft >= 0 && currentDayLeft <= (weeksToShow * 7 * dayWidth)" class="current-day-line"
                :style="{ left: (currentDayLeft + 'px') }"></div>
...
import GanttTaskOT from "@/components/GanttOT/GanttTaskOTFeature.vue";
import cloneDeep from "lodash.clonedeep";
import TaskModal from "@/components/GanttOT/TaskModalFeature.vue";


export default {
  components: {
    Navbar, Sidebar, SubtaskEditor, GanttTaskOT, TaskModal, NotificationBell
  },
  data() {
    return {
  ...
  mounted() {
    ...
    // Llama al método para ajustar el ancho inicialmente
    this.updateGridOffset();

    // Agrega un listener de resize en window y observa el contenedor de días
    window.addEventListener('resize', this.updateGridOffset);
    if (this.$refs.daysGrid) {
      this.resizeObserver = new ResizeObserver(() => {
        this.updateGridOffset();
      });
      this.resizeObserver.observe(this.$refs.daysGrid);
    }
  },
  beforeDestroy() {
  ...
  computed: {
    ...
    currentDayLeft() {
      // Get today's date (March 24)
      const today = new Date();
      // Force the date to be March 24 for testing
      // today.setDate(24);
      // today.setMonth(2); // March (0-indexed)
...
.current-day-line {
  ...
  bottom: 0;
  width: 4px;
  background: linear-gradient(to bottom, rgba(255, 0, 0, 0.9), rgba(255, 0, 0, 0.7));
  z-index: 25;
  /* Aumentado para estar por encima de la cabecera */
}
